...
BEGIN
    ...
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.user_music_tracks;
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
END $$;

-- ==============================================================================
-- 9. Music Tracks Table (Ambient & YouTube Streams)
-- ==============================================================================
CREATE TABLE IF NOT EXISTS public.user_music_tracks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    title TEXT NOT NULL,
    artist TEXT DEFAULT 'Web Stream',
    url TEXT NOT NULL,
    youtube_id TEXT,
    thumbnail_url TEXT,
...
CREATE INDEX IF NOT EXISTS idx_music_tracks_user_id ON public.user_music_tracks(user_id);
...
CREATE POLICY "Users can view their own music tracks" ON public.user_music_tracks FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own music tracks" ON public.user_music_tracks;
CREATE POLICY "Users can insert their own music tracks" ON public.user_music_tracks FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own music tracks" ON public.user_music_tracks;
CREATE POLICY "Users can update their own music tracks" ON public.user_music_tracks FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own music tracks" ON public.user_music_tracks;
CREATE POLICY "Users can delete their own music tracks" ON public.user_music_tracks FOR DELETE USING (auth.uid() = user_id);
