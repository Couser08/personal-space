...
export const ClassicVinylPreset: React.FC<PresetProps> = ({
  currentTrack,
  isPlaying,
  isYouTube,
  currentTime,
  ...
  return (
    <div className="bg-[#242A27] text-[#FAFBF9] rounded-[32px] p-6 border-2 border-[#37443D] shadow-2xl space-y-4 select-none relative overflow-hidden">
      {/* Botanical Amber Aura */}
      <div className={`absolute top-0 right-0 w-40 h-40 bg-[#6BAA7A]/15 rounded-full blur-3xl pointer-events-none transition-opacity duration-700 ${isPlaying ? 'opacity-100' : 'opacity-30'}`} />

      {/* Top Deck Info Bar */}
      <div className="flex items-center justify-between border-b border-[#35433B] pb-2.5">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${isPlaying ? 'bg-[#6BAA7A] shadow-[0_0_8px_#6BAA7A]' : 'bg-[#E0A96D]'}`} />
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[#181D1A] border border-[#37443D] text-[9px] font-mono text-[#6BAA7A]">
            <Radio className={`w-2.5 h-2.5 ${isPlaying ? 'animate-spin' : ''}`} />
            <span>{isPlaying ? 'TURNING' : 'IDLE'}</span>
          </div>

          {/* Sleek Volume Popover Trigger */}
          <div className="relative">
            <button
              ...
              className={`p-1.5 rounded-xl border transition-all cursor-pointer ${
                showVolumePopup
                  ? 'bg-[#6BAA7A] text-[#121614] border-[#6BAA7A]'
                  : 'bg-[#181D1A] text-[#A8B6AE] hover:text-white border-[#37443D]'
              }`}
            >
            ...
            <AnimatePresence>
              {showVolumePopup && (
                <motion.div
                  ...
                  className="absolute right-0 top-8 z-30 bg-[#1A211D] border border-[#37443D] p-3 rounded-2xl shadow-xl w-44 space-y-2"
                >
                  <div className="flex items-center justify-between text-[10px] font-mono text-[#A8B6AE]">
                    <span>Volume</span>
                    <span>{isMuted ? 'Muted' : `${Math.round(volume * 100)}%`}</span>
                  </div>
                  <div className="relative w-full h-2 flex items-center">
                    <div className="w-full h-1.5 bg-[#121614] rounded-full overflow-hidden border border-[#37443D]/50">
                      <div
      ...
      {/* Themed Turntable Scrubber (No Raw Black Line) */}
      <div className="space-y-1.5 pt-1">
        <div className="relative w-full h-3 flex items-center group cursor-pointer">
          {/* Groove Trough Track */}
          <div className="w-full h-1.5 bg-[#181E1B] rounded-full overflow-hidden border border-[#35433B]/80 shadow-inner">
            <div
              className="h-full bg-gradient-to-r from-[#558E63] via-[#6BAA7A] to-[#88C496] rounded-full transition-all duration-100 shadow-[0_0_8px_rgba(107,170,122,0.4)]"
              style={{ width: `${progressPercent}%` }}
            />
          </div>

          {/* Brass Needle Thumb Indicator */}
          <div
            className="absolute w-3 h-3 rounded-full bg-[#C4A97D] border-2 border-[#181E1B] shadow-md pointer-events-none -translate-x-1/2 transition-all duration-100"
